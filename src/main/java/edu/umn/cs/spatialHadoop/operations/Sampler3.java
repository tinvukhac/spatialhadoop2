...
public class Sampler3 {
	...
	interface TextConverter {
		...
		/**
		 * Convert the given text value in place. The return value indicates whether the
		 * conversion was successful or not. If the conversion is not successful, e.g.,
		 * the input is in an incorrect format, the caller should discard the text
		 * value.
		 * 
		 * @param inout
	...
	static class Identity implements TextConverter {
		...
		@Override
		public boolean convert(Text inout) {
			return true;
		}
	}
	...
	public static class ConverterMap extends Mapper<Object, Text, Object, Text> {
		...
		protected void setup(Context context) throws IOException, InterruptedException {
			...
			Class<? extends TextConverter> convClass = conf.getClass(ConverterClass, Identity.class,
					TextConverter.class);
			try {
	...
	public static String[] takeSample(Path[] files, OperationsParams params)
		...
		// Read the file back
		Path tempSample = new Path(tempPath, "sample");
		FSDataOutputStream out = fs.create(tempSample);
		out.close();
		FileStatus[] allFiles = fs.listStatus(tempPath, new PathFilter() {
			@Override
		...
		for(int i = 0; i < allPaths.length; i++) {
			allPaths[i] = allFiles[i].getPath();
		}
		FileUtil.concat(job.getConfiguration(), fs, tempSample, allPaths);
		String[] lines = Head.head(fs, tempSample, outputSize);
	...
	private static void printUsage() {
		...
		System.out.println(
				"outshape:<s> - Shapes to write to output. If not specified, sampled records are not converted.");
